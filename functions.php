 ...
 * Handles both default and custom uploads paths
 */
...
/**
 * Custom upload subdirectories for organization
 */
...
function omega_design_create_upload_directories() {
    ...
    $fallback_image = OMEGA_DESIGN_UPLOADS_IMAGES_DIR . '/fallback.jpg';
    if ( ! file_exists( $fallback_image ) ) {
        // Create a simple fallback image (optional)
        // wp_remote_get( OMEGA_DESIGN_IMAGES_URI . '/fallback.jpg' );
    }
}
add_action( 'after_switch_theme', 'omega_design_create_upload_directories' );

/**
 * Get upload directory path with optional subdirectory
 * 
...
/**
 * Get upload directory URL with optional subdirectory
 * 
 * @param string $subdir Optional subdirectory
 ...
 * 
 * @param string $file_data File content
 * @param string $filename File name
 * @param string $subdir Subdirectory
 * @return string|false Saved file path or false
 */
...
$loader_file = OMEGA_DESIGN_INCLUDES . '/core/loader.php';
if ( ! file_exists( $loader_file ) ) {
    wp_die( sprintf( 'Core loader file not found: %s', $loader_file ) );
}
...
function omega_design_initialize() {
    if ( class_exists( '\OmegaDesign\Core\Loader' ) ) {
        return \OmegaDesign\Core\Loader::instance();
    }
    return null;
}
